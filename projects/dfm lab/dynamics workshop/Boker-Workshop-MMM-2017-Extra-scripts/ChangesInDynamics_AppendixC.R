# ---------------------------------------------------------------------
# Program: ChangesInDynamics_AppendixC.R
#  Author: Steve Boker
#    Date: Wed Jan 27 13:49:24 EST 2016
#
...
tDataFrame <- read.csv(file="ExampleDataAppendixC.csv", header=TRUE)
...
pdf("TimeSeriesAppendixC.pdf", height=5, width=6)
...
multilevelLDE2 <- mxModel("multilevelLDE2", 
...
multilevelLDE2Fit <- mxRun(multilevelLDE2)

summary(multilevelLDE2Fit)

# ----------------------------------
# Examine the individual differences results.

RandOut <- mxEval(Rand, model=multilevelLDE2Fit)
...
pdf("TimeSeriesAppendixC-fitted.pdf", height=5, width=6)
plot(c(1, numOccasions), c(-15, 15),
     xlab="Sample Index",
     ylab="Score",
     type='n')
