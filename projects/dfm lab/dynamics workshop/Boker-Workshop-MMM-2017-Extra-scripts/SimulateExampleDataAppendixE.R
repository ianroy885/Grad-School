# ---------------------------------------------------------------------
# Program: SimulateExampleDataAppendixE.R
#  Author: Steve Boker
#    Date: Wed Jan 27 13:58:10 EST 2016
#
# This program simulates individual differences in equilibrium level and slope
#   with a second level predictor of frequency and damping. 
#
# ---------------------------------------------------------------------
...
for(tID in 1:N) {
    ...
    eta <- meanEta + 0.1 * z2[tID]
    zeta <- meanZeta + 0.1 * z2[tID]
    randInt <-  sdInt * z3[tID]
    randSlope <-  sdSlope * z3[tID]
...
pdf("ExampleDataAppendixD.pdf", height=5, width=6)
plot(c(1, P), c(-10, 10),
     xlab="Sample Index",
     ylab="Score",
     type='n')
...
write.csv(simDataFrame, file="ExampleDataAppendixE.csv", row.names=FALSE)
