...
#'
#' @description \link{lsirm1pl_fixed_gamma_mcar} is used to fit LSIRM with gamma fixed to 1 in incomplete data assumed to be missing completely at random.
#'  \link{lsirm1pl_fixed_gamma_mcar} factorizes item response matrix into column-wise item effect, row-wise respondent effect and further embeds interaction effect in a latent space, while ignoring the missing element under the assumption of missing completely at random. The resulting latent space provides an interaction map that represents interactions between respondents and items.
#'
...
#' }
#' @export
lsirm1pl_fixed_gamma_mcar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <- lsirm1pl_fixed_gamma_mcar_cpp(data=as.matrix(data), ndim=ndim, niter=niter, nburn=nburn, nthin=nthin, nprint=nprint,
                           ...
                           pr_sd_theta=pr_sd_theta, pr_a_theta=pr_a_theta, pr_b_theta=pr_b_theta, missing=missing.val, verbose=verbose, fix_theta_sd=fix_theta_sd)
...
cat("\n")
  ...
  # Calculate BIC
  # cat("\n\nCalculate BIC\n")
  log_like = log_likelihood_cpp(as.matrix(data), ndim, as.matrix(beta.estimate), as.matrix(theta.estimate), 1, z.est, w.est, missing.val)
