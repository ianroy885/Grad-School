...
#'
#'
#' @description \link{lsirm1pl_normal_mcar} is used to fit LSIRM with 1pl in incomplete data assumed to be missing completely at random.
...
#'
#' @return \code{lsirm1pl_normal_mcar} returns an object of  list containing the following components:
...
#'
#' @export
lsirm1pl_normal_mcar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
                                ...
                                pr_a_theta = 0.001, pr_b_theta = 0.001, pr_a_eps = 0.001, pr_b_eps = 0.001, missing.val = 99, verbose=FALSE, fix_theta_sd=FALSE){
  if(niter < nburn){
  ...
  output <- lsirm1pl_normal_mcar_cpp(data = as.matrix(data), ndim=ndim, niter=niter, nburn=nburn, nthin=nthin, nprint=nprint,
                                     ...
                                     missing=missing.val, verbose=verbose, fix_theta_sd=fix_theta_sd)
