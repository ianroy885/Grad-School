...
#' @description \link{lsirm1pl_mcar_ss} is used to fit 1PL LSIRM with model selection approach based on spike-and-slab priors in incomplete data assumed to be missing completely at random.
...
#'
#' @return \code{lsirm1pl_mcar_ss} returns an object of  list containing the following components:
...
#' }
#' @export
lsirm1pl_mcar_ss = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <- lsirm1pl_mcar_ss_cpp(data=as.matrix(data), ndim=ndim, niter=niter, nburn=nburn, nthin=nthin, nprint=nprint,
