...
#'
#'
#' @description \link{lsirm2pl_mar_ss} is used to fit 2PL LSIRM based on spike-and-slab priors in incomplete data assumed to be missing completely at random.
#' \link{lsirm2pl_mar_ss} factorizes item response matrix into column-wise item effect, row-wise respondent effect in a latent space, while ignoring the missing element under the assumption of missing completely at random. Unlike 1PL model, 2PL model assumes the item effect can vary according to respondent, allowing additional parameter multiplied with respondent effect.  The resulting latent space provides an interaction map that represents interactions between respondents and items.
#'
...
#' @param pr_slab_sd Numeric; the standard deviation of spike prior for log gamma. Default is 1.
...
cat("\n")
  ...
  if(pi.estimate > 0.5){
    ...
    log_like = log_likelihood_2pl_cpp(as.matrix(data), ndim, as.matrix(beta.estimate), as.matrix(alpha.estimate), as.matrix(theta.estimate), 0, z.est, w.est, missing.val)
  }
